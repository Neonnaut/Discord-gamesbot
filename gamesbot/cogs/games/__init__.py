...
class Games(commands.Cog, name='games'):
    ...
    async def choose(self, ctx: commands.Context, *, choices):
        """Chooses a random option from multiple choices.
        Example: `|p|choose "Helena Rubinstein" Cher "Margot Robbie" Dave`"""
            ...
            if die.isdigit() and amount.isdigit():
                ...
                if 1 <= amount <= 100:
                    if 2 <= die <= 100:
                        ...
                        errorMessage.append(
                            'The die must be in the range 2 to 100')
                else:
                    errorMessage.append(
                        'The amount of dice must be in the range 1 to 100')
            else:
                errorMessage.append(
                    'The requested dice roll was not in the format NdN')
        elif dice.isdigit():
            die = int(dice)
            dice = f'1d{dice}'
            if 2 <= die <= 100:
                ...
                errorMessage.append(
                    'The amount of dice must be in the range 1 to 10')
        else:
            errorMessage.append(
                'The requested dice roll was not in the format NdN')
        ...
        if myModifier != None:
            if len(myModifier) < 6:
                try:
                    ...
                    myModifier = 0
                    errorMessage.append(
                        'Modifiers should look like `+ 300` or `- 20`')
            else:
                myModifier = 0
